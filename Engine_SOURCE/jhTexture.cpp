...
namespace jh::graphics
{
	...
	void Texture::LoadFile(const std::wstring& fullPath)
	{
		...
		{
			if (FAILED(LoadFromDDSFile(fullPath.c_str(), DDS_FLAGS_NONE, nullptr, mImage)))
				return;
		}
		...
		{
			if (FAILED(LoadFromWICFile(fullPath.c_str(), WIC_FLAGS_NONE, nullptr, mImage)))
				return;
		}
	}
	...
	{
		CreateShaderResourceView(GetDevice()->GetID3D11Device(), mImage.GetImages(),
